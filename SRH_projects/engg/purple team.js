...
//3. match players height more than 2 feet
Match (player:PLAYER) -[:PLAYS_FOR]->(team:TEAM)
where player.height >=2
return player

//4. find max height among all players
Match (player:PLAYER) 
return max(player.height)

//5. Player name with max height
Match (player:PLAYER) 
with max(player.height) as mx
match (player:PLAYER) where player.height=mx
return player.name,player.height

//6. Player with minimum age
Match (player:PLAYER)
with min(player.age) as young
match (player:PLAYER) where player.age=young
return player.name,player.age

//7. Player who plays for which team
Match (player:PLAYER)-[:PLAYS_FOR]-> (team:TEAM)
return player.name,team.name

//8. players descending by "number" property
Match (player:PLAYER)
return player.name, player.number 
order by player.number DESC

//9. Skip and Limit
match (p:PLAYER)-[:PLAYED_AGAINST]->(t:TEAM)
...
match (p:PLAYER)
where p.age>30 and p.height>2
return p.name,p.age,p.height

//12. who is the coach for which team
match (c:COACH)-[cf :COACHES_FOR]->(t:TEAM)
return c.name,t.name

//13. multiple parameter sorting using keys function
match (p:PLAYER)
return p.name,p.age,p.height
order by keys(p)

//14. 
match (p:PLAYER)-[t:TEAMMATES]->(p2:PLAYER)
